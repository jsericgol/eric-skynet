...
function gateserver.start(handler)
	skynet.error("=====gateserver.start")
	assert(handler.message)
	assert(handler.connect)

	function CMD.open( source, conf )
		skynet.error("=======gateserver.lua=> CMD.open: source=", source)
		if conf ~= nil then
			local str1 = "=======gateserver.lua=> CMD.open:"
			local str2 =
				string.format("port=%d,maxclient=%d,nodelay=%s",
							  conf.port, conf.maxclient, conf.nodelay)
			skynet.error(str1 .. str2)
		end
		assert(not socket)
		...
		skynet.error(string.format("gateserver.lua=>gateserver listen on %s:%d", address, port))
	...
	local function dispatch_msg(fd, msg, sz)
		skynet.error("========gateserver.lua=> dispatch_msg: fd=", fd)
		if connection[fd] then
	...
	local function dispatch_queue()
		skynet.error("========gateserver.lua=> dispatch_queue")
	...
	function MSG.open(fd, msg)
		local str = string.format("fd=%d, msg=%s", fd, msg)
		skynet.error("========gateserver.lua=> MSG.open:" .. str)
	...
	skynet.register_protocol {
		...
		dispatch = function (_, _, q, type, ...)
			queue = q
			if type then
				skynet.error("gateserver.lua=>skynet.register_protocol:dispatch, type=", type)
				MSG[type](...)
			end
		end
	}
